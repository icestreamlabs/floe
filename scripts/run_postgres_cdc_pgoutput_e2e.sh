...
CONTAINER_NAME="${CONTAINER_NAME:-floe-postgres-cdc-e2e}"
...
echo "Running native pgoutput CDC e2e test"
TEST_ARGS=(-p floe-cdc-pg --test postgres_pgoutput_e2e -- --ignored --nocapture)
if [[ "${FLOE_TEST_RELEASE}" == "1" ]]; then
  TEST_ARGS=(--release "${TEST_ARGS[@]}")
fi

FLOE_CDC_PG_HOST="127.0.0.1" \
FLOE_CDC_PG_PORT="${POSTGRES_PORT}" \
FLOE_CDC_PG_USER="${POSTGRES_USER}" \
FLOE_CDC_PG_PASSWORD="${POSTGRES_PASSWORD}" \
FLOE_CDC_PG_DATABASE="${POSTGRES_DB}" \
