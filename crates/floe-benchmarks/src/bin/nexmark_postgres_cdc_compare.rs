...
fn query_sql_risingwave(query_id: &str) -> Option<String> {
    query_sql_portable(query_id)
}

fn expected_sql_for_engine(engine: Engine, query_id: &str) -> Option<String> {
    match engine {
...
fn query_sql_portable(query_id: &str) -> Option<String> {
    nexmark_query_sql(query_id, NexmarkSqlDialect::Portable)
}

fn query_sql_floe(query_id: &str) -> Option<String> {
    nexmark_query_sql(query_id, NexmarkSqlDialect::FloeCdc)
}

fn query_sql_expected_for_floe(query_id: &str) -> Option<String> {
    nexmark_query_sql(query_id, NexmarkSqlDialect::PostgresExpected)
}
...
mod tests {
    ...
    const RANK2_AUCTIONS: &str = "COUNT(DISTINCT auction) FILTER (WHERE price >= 10000 AND price < 1000000) AS rank2_auctions";

    #[test]
    fn q15_and_q16_preserve_canonical_price_tiers_in_every_dialect() {
        for query_id in ["q15", "q16"] {
            for sql in [
                query_sql_portable(query_id),
                query_sql_floe(query_id),
                query_sql_expected_for_floe(query_id),
            ] {
                let sql = sql.expect("query dialect");
                assert!(
                    sql.contains(RANK2_AUCTIONS),
                    "{query_id} lost the canonical rank2 auction predicate: {sql}"
                );
            }
        }
    }
