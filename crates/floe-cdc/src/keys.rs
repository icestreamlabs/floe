...
pub(crate) fn source_metadata_key(source_id: &CdcSourceId) -> Result<Vec<u8>> {
    let mut key = source_metadata_prefix();
    push_component(&mut key, source_id.as_str().as_bytes())?;
    Ok(key)
}

pub(crate) fn table_metadata_key(table_id: &CdcTableId) -> Result<Vec<u8>> {
    let mut key = CDC_PREFIX.to_vec();
...
pub(crate) fn source_table_index_prefix(source_id: &CdcSourceId) -> Result<Vec<u8>> {
    let mut key = CDC_PREFIX.to_vec();
...
pub(crate) fn source_table_index_key(
...
pub(crate) fn checkpoint_key(source_id: &CdcSourceId) -> Result<Vec<u8>> {
    let mut key = CDC_PREFIX.to_vec();
...
fn push_component(out: &mut Vec<u8>, component: &[u8]) -> Result<()> {
    let len = u32::try_from(component.len())
        .map_err(|_| anyhow!("CDC key component length exceeds u32"))?;
