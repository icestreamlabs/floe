...
impl DbspPartitionedTop1 {
    ...
    pub async fn new_with_batch_key_and_order_extractor<K, P, O, F, G>(
        input: &DeltaHandleStream,
        key_extractor: F,
        order_extractor: G,
        error_handler: Option<RuntimeErrorHandler>,
    ...
    where
        ...
        O: Clone + Ord + Send + Sync + 'static,
        F: Fn(&[(K, i64)]) -> Vec<(K, i64, Option<P>, Option<O>)> + Send + Sync + Clone + 'static,
        G: Fn(&[(K, i64)]) -> Vec<(K, i64, Option<OrderedBytes>)> + Send + Sync + Clone + 'static,
    {
        let table = input.table();
        let frontier = input.current_time();
        let horizon = input.semantic_horizon();
        ...
        let input_index = IndexedBatchZSet::new(table.clone(), format!("top1_input_{top1_id}"));
        let ordered_index =
            IndexedBatchZSet::with_range_index(table.clone(), format!("top1_ordered_{top1_id}"));
        ordered_index
            .restore_committed_checkpoint()
            .await
            .context("restore committed top1 ordered index")?;
        let key_extractor = Arc::new(key_extractor);
        let order_extractor = Arc::new(order_extractor);
        let top1_op = Arc::new(AsyncMutex::new(
            PartitionedTop1Op::new_with_batch_key_and_order_extractor(
                input_index,
                ordered_index,
                table.clone(),
                output,
