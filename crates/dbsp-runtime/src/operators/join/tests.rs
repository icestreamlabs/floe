...
async fn join_operator_matches_batch_join_over_time() {
    ...
    let left_index = LegacyIndexedBatchZSet::new(table.clone(), "join_left_index");
    let right_index = LegacyIndexedBatchZSet::new(table.clone(), "join_right_index");
...
async fn join_operator_handles_negative_deltas() {
    ...
    let left_index = LegacyIndexedBatchZSet::new(table.clone(), "neg_left_index");
    let right_index = LegacyIndexedBatchZSet::new(table.clone(), "neg_right_index");
    let left_key = Arc::new(|value: &i64| Some(*value));
    let right_key = Arc::new(|value: &i64| Some(*value));
...
async fn join_operator_skips_null_keys() {
    ...
    let output = VersionedZSet::new(out_dict.clone(), table.clone(), "null_output".to_string())
        .await
        .expect("output");
    let left_index = LegacyIndexedBatchZSet::new(table.clone(), "null_left_index");
    let right_index = LegacyIndexedBatchZSet::new(table.clone(), "null_right_index");
...
async fn join_operator_matches_full_recompute() {
    ...
    let left_index = LegacyIndexedBatchZSet::new(table.clone(), "recompute_left_index");
    let right_index = LegacyIndexedBatchZSet::new(table.clone(), "recompute_right_index");
    let left_key = Arc::new(|value: &i64| Some(*value));
    let right_key = Arc::new(|value: &i64| Some(*value));
