    ...
    A::Archived: RkyvDeserialize<A, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
{
    async fn on_step(
        ...
        let new_integrated_handle =
            Self::apply_deltas_to_versioned(&mut self.state.integrated, &output_deltas, base_version)
                .await
        ...
        let delta_handle =
            Self::apply_deltas_to_versioned(&mut self.output, &output_deltas, None)
                .await
...
mod tests {
    ...
    async fn group_by_updates_aggregates_by_key() {
        ...
        let index = IndexedZSet::new(table.clone(), "group_by_index");
