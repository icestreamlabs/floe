use std::collections::{BTreeMap, HashMap, HashSet};
use std::hash::Hash;
use std::sync::Arc;

use anyhow::{Context, Result};
use async_trait::async_trait;
use rkyv::Archive;
use rkyv::Deserialize as RkyvDeserialize;
use rkyv::Serialize as RkyvSerialize;
use rkyv::bytecheck::CheckBytes;
use slatedb::WriteBatch;

use crate::collections::LegacyIndexedBatchZSet;
use crate::collections::zset::{SegmentRecord, VersionedZSet};
use crate::handles::ZSetHandle;
use crate::relation_state::RelationState;
use crate::storage::KeyValueTable;
use crate::storage::dictionary::Dictionary;
use crate::storage::encoding::{RkyvDeserializer, RkyvSerializer, RkyvValidator};
use crate::stream::runtime::DeltaOperator;
use crate::stream::util::delta_zset_handle;

type JoinKeyExtractor<T, K> = Arc<dyn Fn(&T) -> Option<K> + Send + Sync>;
type TimestampExtractor<T, TS> = Arc<dyn Fn(&T) -> TS + Send + Sync>;
type JoinProjector<L, R, O> = Arc<dyn Fn(&L, Option<&R>) -> O + Send + Sync>;

pub struct AsofJoinOp<L, R, O, K, TS>
where
    L: Archive
        + Clone
        + Eq
        + Hash
        + Send
        + Sync
        + 'static
        + for<'a> RkyvSerialize<RkyvSerializer<'a>>,
    L::Archived: RkyvDeserialize<L, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
    R: Archive
        + Clone
        + Eq
        + Hash
        + Ord
        + Send
        + Sync
        + 'static
        + for<'a> RkyvSerialize<RkyvSerializer<'a>>,
    R::Archived: RkyvDeserialize<R, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
    O: Archive
        + Clone
        + Eq
        + Hash
        + Send
        + Sync
        + 'static
        + for<'a> RkyvSerialize<RkyvSerializer<'a>>,
    O::Archived: RkyvDeserialize<O, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
    K: Archive
        + Clone
        + Eq
        + Hash
        + Send
        + Sync
        + 'static
        + for<'a> RkyvSerialize<RkyvSerializer<'a>>,
    K::Archived: RkyvDeserialize<K, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
    TS: Ord + Clone + Send + Sync + 'static,
{
    pub left_state: RelationState<L>,
    pub right_state: RelationState<R>,
    pub left_index: LegacyIndexedBatchZSet<K, L>,
    pub right_index: LegacyIndexedBatchZSet<K, R>,
    pub left_key: JoinKeyExtractor<L, K>,
    pub right_key: JoinKeyExtractor<R, K>,
    pub left_ts: TimestampExtractor<L, TS>,
    pub right_ts: TimestampExtractor<R, TS>,
    pub projector: JoinProjector<L, R, O>,
    pub table: Arc<dyn KeyValueTable>,
    pub integrated: Option<RelationState<O>>,
    output: VersionedZSet<O>,
    dict_cache_left: HashMap<String, Arc<Dictionary<L>>>,
    dict_cache_right: HashMap<String, Arc<Dictionary<R>>>,
}

impl<L, R, O, K, TS> AsofJoinOp<L, R, O, K, TS>
where
    L: Archive
        + Clone
        + Eq
        + Hash
        + Send
        + Sync
        + 'static
        + for<'a> RkyvSerialize<RkyvSerializer<'a>>,
    L::Archived: RkyvDeserialize<L, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
    R: Archive
        + Clone
        + Eq
        + Hash
        + Ord
        + Send
        + Sync
        + 'static
        + for<'a> RkyvSerialize<RkyvSerializer<'a>>,
    R::Archived: RkyvDeserialize<R, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
    O: Archive
        + Clone
        + Eq
        + Hash
        + Send
        + Sync
        + 'static
        + for<'a> RkyvSerialize<RkyvSerializer<'a>>,
    O::Archived: RkyvDeserialize<O, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
    K: Archive
        + Clone
        + Eq
        + Hash
        + Send
        + Sync
        + 'static
        + for<'a> RkyvSerialize<RkyvSerializer<'a>>,
    K::Archived: RkyvDeserialize<K, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
    TS: Ord + Clone + Send + Sync + 'static,
{
    #[allow(clippy::too_many_arguments)]
    pub fn new(
        left_state: RelationState<L>,
        right_state: RelationState<R>,
        left_index: LegacyIndexedBatchZSet<K, L>,
        right_index: LegacyIndexedBatchZSet<K, R>,
        left_key: JoinKeyExtractor<L, K>,
        right_key: JoinKeyExtractor<R, K>,
        left_ts: TimestampExtractor<L, TS>,
        right_ts: TimestampExtractor<R, TS>,
        projector: JoinProjector<L, R, O>,
        table: Arc<dyn KeyValueTable>,
        output: VersionedZSet<O>,
        integrated: Option<RelationState<O>>,
    ) -> Self {
        Self {
            left_state,
            right_state,
            left_index,
            right_index,
            left_key,
            right_key,
            left_ts,
            right_ts,
            projector,
            table,
            integrated,
            output,
            dict_cache_left: HashMap::new(),
            dict_cache_right: HashMap::new(),
        }
    }

    fn coalesce_entries<T>(&self, entries: Vec<(T, i64)>) -> HashMap<T, i64>
    where
        T: Clone + Eq + Hash,
    {
        let mut merged = HashMap::new();
        for (row, weight) in entries {
            let entry = merged.entry(row.clone()).or_insert(0);
            *entry += weight;
            if *entry == 0 {
                merged.remove(&row);
            }
        }
        merged
    }

    fn apply_entry_deltas<T>(
        &self,
        base: &HashMap<T, i64>,
        deltas: Option<&Vec<(T, i64)>>,
    ) -> HashMap<T, i64>
    where
        T: Clone + Eq + Hash,
    {
        let mut next = base.clone();
        if let Some(deltas) = deltas {
            for (row, delta) in deltas {
                let entry = next.entry(row.clone()).or_insert(0);
                *entry += *delta;
                if *entry == 0 {
                    next.remove(row);
                }
            }
        }
        next
    }

    fn asof_output_for_key(
        &self,
        left: &HashMap<L, i64>,
        right: &HashMap<R, i64>,
    ) -> HashMap<O, i64> {
        let mut output = HashMap::new();

        let mut right_items: Vec<(TS, R, i64)> = right
            .iter()
            .filter_map(|(row, weight)| {
                if *weight == 0 {
                    None
                } else {
                    Some(((self.right_ts)(row), row.clone(), *weight))
                }
            })
            .collect();
        right_items.sort_by(|(ts_a, row_a, _), (ts_b, row_b, _)| {
            ts_a.cmp(ts_b).then_with(|| row_a.cmp(row_b))
        });

        for (left_row, left_weight) in left {
            if *left_weight == 0 {
                continue;
            }
            let left_ts = (self.left_ts)(left_row);
            let candidate = if right_items.is_empty() {
                None
            } else {
                let pos = right_items.partition_point(|(ts, _, _)| ts <= &left_ts);
                if pos == 0 {
                    None
                } else {
                    Some(&right_items[pos - 1])
                }
            };

            let out = (self.projector)(left_row, candidate.map(|(_, row, _)| row));
            let weight = match candidate {
                Some((_, _, right_weight)) => left_weight * right_weight,
                None => *left_weight,
            };
            let entry = output.entry(out.clone()).or_insert(0);
            *entry += weight;
            if *entry == 0 {
                output.remove(&out);
            }
        }

        output.retain(|_, weight| *weight != 0);
        output
    }

    async fn apply_deltas_to_versioned<T>(
        versioned: &mut VersionedZSet<T>,
        deltas: &HashMap<T, i64>,
        base: Option<u64>,
    ) -> Result<ZSetHandle>
    where
        T: Archive
            + Clone
            + Eq
            + Hash
            + Send
            + Sync
            + 'static
            + for<'a> RkyvSerialize<RkyvSerializer<'a>>,
        T::Archived: RkyvDeserialize<T, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
    {
        let mut buckets: BTreeMap<u16, Vec<(u64, i64)>> = BTreeMap::new();
        let dict = versioned.dictionary();
        let mut dict_batch = dict.batch();
        for (key, delta) in deltas {
            if *delta == 0 {
                continue;
            }
            let id = dict_batch
                .intern(key)
                .await
                .context("intern key while staging asof join delta")?;
            buckets
                .entry(bucket_for(id))
                .or_default()
                .push((id, *delta));
        }
        drop(dict_batch);

        let mut segments = Vec::new();
        for (bucket, mut bucket_deltas) in buckets {
            bucket_deltas.retain(|(_, delta)| *delta != 0);
            if bucket_deltas.is_empty() {
                continue;
            }
            bucket_deltas.sort_by_key(|(id, _)| *id);
            segments.push(SegmentRecord {
                id: 0,
                bucket,
                deltas: bucket_deltas,
            });
        }

        if segments.is_empty() {
            if base.is_some()
                && let Some(handle) = versioned.current_handle()
            {
                return Ok(handle);
            }
            return Ok(versioned.handle_for_version(0));
        }

        let mut batch = WriteBatch::new();
        let plan = versioned
            .enqueue_version_with_base(segments, base, 0, &mut batch)
            .await
            .context("schedule asof join version update")?;

        versioned
            .table()
            .write_batch(batch)
            .await
            .context("write asof join version update")?;

        let mut cleanup = WriteBatch::new();
        cleanup.delete(versioned.intent_key_bytes());
        versioned
            .table()
            .write_batch(cleanup)
            .await
            .context("clear asof join intent")?;

        versioned.apply_version_plan(&plan);
        Ok(versioned.handle_for_version(plan.version))
    }
}

#[async_trait]
impl<L, R, O, K, TS> DeltaOperator for AsofJoinOp<L, R, O, K, TS>
where
    L: Archive
        + Clone
        + Eq
        + Hash
        + Send
        + Sync
        + 'static
        + for<'a> RkyvSerialize<RkyvSerializer<'a>>,
    L::Archived: RkyvDeserialize<L, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
    R: Archive
        + Clone
        + Eq
        + Hash
        + Ord
        + Send
        + Sync
        + 'static
        + for<'a> RkyvSerialize<RkyvSerializer<'a>>,
    R::Archived: RkyvDeserialize<R, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
    O: Archive
        + Clone
        + Eq
        + Hash
        + Send
        + Sync
        + 'static
        + for<'a> RkyvSerialize<RkyvSerializer<'a>>,
    O::Archived: RkyvDeserialize<O, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
    K: Archive
        + Clone
        + Eq
        + Hash
        + Send
        + Sync
        + 'static
        + for<'a> RkyvSerialize<RkyvSerializer<'a>>,
    K::Archived: RkyvDeserialize<K, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
    TS: Ord + Clone + Send + Sync + 'static,
{
    async fn on_step(
        &mut self,
        _ts: i64,
        inputs: &[ZSetHandle],
    ) -> anyhow::Result<Option<ZSetHandle>> {
        let left_delta_handle = inputs
            .first()
            .cloned()
            .context("asof join requires left delta handle")?;
        let right_delta_handle = inputs
            .get(1)
            .cloned()
            .context("asof join requires right delta handle")?;

        let left_delta_values = delta_zset_handle::<L>(
            self.table.clone(),
            &mut self.dict_cache_left,
            &left_delta_handle,
        )
        .await
        .context("load left delta for asof join")?;
        let right_delta_values = delta_zset_handle::<R>(
            self.table.clone(),
            &mut self.dict_cache_right,
            &right_delta_handle,
        )
        .await
        .context("load right delta for asof join")?;

        let left_delta = self.coalesce_entries(left_delta_values);
        let right_delta = self.coalesce_entries(right_delta_values);

        let mut left_keyed: HashMap<K, Vec<(L, i64)>> = HashMap::new();
        for (row, weight) in &left_delta {
            if *weight == 0 {
                continue;
            }
            if let Some(key) = (self.left_key)(row) {
                left_keyed
                    .entry(key)
                    .or_insert_with(Vec::new)
                    .push((row.clone(), *weight));
            }
        }

        let mut right_keyed: HashMap<K, Vec<(R, i64)>> = HashMap::new();
        for (row, weight) in &right_delta {
            if *weight == 0 {
                continue;
            }
            if let Some(key) = (self.right_key)(row) {
                right_keyed
                    .entry(key)
                    .or_insert_with(Vec::new)
                    .push((row.clone(), *weight));
            }
        }

        let mut affected_keys: HashSet<K> = HashSet::new();
        affected_keys.extend(left_keyed.keys().cloned());
        affected_keys.extend(right_keyed.keys().cloned());

        let mut delta_output: HashMap<O, i64> = HashMap::new();

        for key in affected_keys {
            let old_left_entries = self
                .left_index
                .values_for_key(&key)
                .await
                .context("load left index for asof join")?;
            let old_right_entries = self
                .right_index
                .values_for_key(&key)
                .await
                .context("load right index for asof join")?;

            let old_left = self.coalesce_entries(old_left_entries);
            let old_right = self.coalesce_entries(old_right_entries);

            let new_left = self.apply_entry_deltas(&old_left, left_keyed.get(&key));
            let new_right = self.apply_entry_deltas(&old_right, right_keyed.get(&key));

            let old_output = self.asof_output_for_key(&old_left, &old_right);
            let new_output = self.asof_output_for_key(&new_left, &new_right);

            let mut key_delta = new_output;
            for (row, weight) in old_output {
                let key = row.clone();
                let entry = key_delta.entry(key.clone()).or_insert(0);
                *entry -= weight;
                if *entry == 0 {
                    key_delta.remove(&key);
                }
            }

            for (row, weight) in key_delta {
                let key = row.clone();
                let entry = delta_output.entry(key.clone()).or_insert(0);
                *entry += weight;
                if *entry == 0 {
                    delta_output.remove(&key);
                }
            }
        }

        let left_base = self
            .left_state
            .integrated
            .current_handle()
            .map(|handle| handle.version);
        let new_left_handle = Self::apply_deltas_to_versioned(
            &mut self.left_state.integrated,
            &left_delta,
            left_base,
        )
        .await
        .context("update left integrated state")?;
        self.left_state.update_handle(new_left_handle);

        let right_base = self
            .right_state
            .integrated
            .current_handle()
            .map(|handle| handle.version);
        let new_right_handle = Self::apply_deltas_to_versioned(
            &mut self.right_state.integrated,
            &right_delta,
            right_base,
        )
        .await
        .context("update right integrated state")?;
        self.right_state.update_handle(new_right_handle);

        let mut left_updates = Vec::new();
        for (key, entries) in &left_keyed {
            for (row, weight) in entries {
                left_updates.push((key.clone(), row.clone(), *weight));
            }
        }
        if !left_updates.is_empty() {
            self.left_index
                .apply_deltas(left_updates)
                .await
                .context("update left asof join index")?;
        }

        let mut right_updates = Vec::new();
        for (key, entries) in &right_keyed {
            for (row, weight) in entries {
                right_updates.push((key.clone(), row.clone(), *weight));
            }
        }
        if !right_updates.is_empty() {
            self.right_index
                .apply_deltas(right_updates)
                .await
                .context("update right asof join index")?;
        }

        if delta_output.is_empty() {
            return Ok(None);
        }

        if let Some(integrated) = &mut self.integrated {
            let base = integrated
                .integrated
                .current_handle()
                .map(|handle| handle.version);
            let new_integrated_handle =
                Self::apply_deltas_to_versioned(&mut integrated.integrated, &delta_output, base)
                    .await
                    .context("update integrated asof join state")?;
            integrated.update_handle(new_integrated_handle);
        }

        let delta_handle = Self::apply_deltas_to_versioned(&mut self.output, &delta_output, None)
            .await
            .context("persist asof join delta output")?;
        Ok(Some(delta_handle))
    }
}

fn bucket_for(id: u64) -> u16 {
    (id >> 48) as u16
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::stream::runtime::DeltaOperator;
    use crate::stream::util::{compute_delta, materialize_zset_handle};
    use object_store::memory::InMemory;
    use slatedb::Db;
    use std::collections::BTreeMap;

    type Row = (i64, i64, i64);
    type Out = (i64, i64, i64, Option<i64>);

    async fn build_db() -> Arc<Db> {
        let store: Arc<dyn object_store::ObjectStore> = Arc::new(InMemory::new());
        Arc::new(Db::open("asof_join", store).await.expect("open SlateDB"))
    }

    async fn stage_version<K>(
        dict: Arc<Dictionary<K>>,
        table: Arc<dyn KeyValueTable>,
        namespace: &str,
        deltas: &[(K, i64)],
    ) -> ZSetHandle
    where
        K: Archive
            + Clone
            + Eq
            + Hash
            + Send
            + Sync
            + 'static
            + for<'rk> RkyvSerialize<RkyvSerializer<'rk>>,
        K::Archived: RkyvDeserialize<K, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
    {
        let mut buckets: BTreeMap<u16, Vec<(u64, i64)>> = BTreeMap::new();
        let mut dict_batch = dict.batch();
        for (key, delta) in deltas {
            let id = dict_batch
                .intern(key)
                .await
                .expect("intern key for asof join test");
            buckets
                .entry(bucket_for(id))
                .or_default()
                .push((id, *delta));
        }
        drop(dict_batch);

        let mut segments = Vec::new();
        for (bucket, mut bucket_deltas) in buckets {
            bucket_deltas.retain(|(_, delta)| *delta != 0);
            if bucket_deltas.is_empty() {
                continue;
            }
            bucket_deltas.sort_by_key(|(id, _)| *id);
            segments.push(SegmentRecord {
                id: 0,
                bucket,
                deltas: bucket_deltas,
            });
        }

        let mut versioned = VersionedZSet::new(dict, table, namespace.to_string())
            .await
            .expect("build versioned");
        let version = versioned
            .create_version_with_base(segments, None)
            .await
            .expect("create version");
        versioned.handle_for_version(version)
    }

    fn apply_deltas<K: Clone + Eq + Hash>(state: &mut HashMap<K, i64>, deltas: &[(K, i64)]) {
        for (key, delta) in deltas {
            let entry = state.entry(key.clone()).or_insert(0);
            *entry += *delta;
            if *entry == 0 {
                state.remove(key);
            }
        }
    }

    fn recompute_asof(left: &HashMap<Row, i64>, right: &HashMap<Row, i64>) -> HashMap<Out, i64> {
        let mut right_by_key: HashMap<i64, HashMap<Row, i64>> = HashMap::new();
        for (row, weight) in right {
            if *weight == 0 {
                continue;
            }
            let entry = right_by_key.entry(row.0).or_insert_with(HashMap::new);
            let value = entry.entry(*row).or_insert(0);
            *value += *weight;
            if *value == 0 {
                entry.remove(row);
            }
        }

        let mut output = HashMap::new();
        for (left_row, left_weight) in left {
            if *left_weight == 0 {
                continue;
            }
            let right_entries = right_by_key.get(&left_row.0);
            let candidate = right_entries.and_then(|entries| {
                let mut best: Option<(&Row, i64)> = None;
                for (row, weight) in entries {
                    if row.1 > left_row.1 {
                        continue;
                    }
                    let replace = match best {
                        None => true,
                        Some((current, _)) => {
                            row.1 > current.1 || (row.1 == current.1 && row > current)
                        }
                    };
                    if replace {
                        best = Some((row, *weight));
                    }
                }
                best
            });

            let out_row = (
                left_row.0,
                left_row.1,
                left_row.2,
                candidate.map(|(row, _)| row.2),
            );
            let weight = match candidate {
                Some((_, right_weight)) => left_weight * right_weight,
                None => *left_weight,
            };
            let entry = output.entry(out_row).or_insert(0);
            *entry += weight;
            if *entry == 0 {
                output.remove(&out_row);
            }
        }

        output
    }

    #[tokio::test]
    async fn asof_join_operator_matches_recompute() {
        let db = build_db().await;
        let table: Arc<dyn KeyValueTable> = Arc::new(crate::storage::SlateTable::new(db.clone()));

        let left_dict = Arc::new(
            Dictionary::<Row>::with_table(table.clone(), "asof_left_stream", None)
                .await
                .expect("left dict"),
        );
        let right_dict = Arc::new(
            Dictionary::<Row>::with_table(table.clone(), "asof_right_stream", None)
                .await
                .expect("right dict"),
        );
        let output_dict = Arc::new(
            Dictionary::<Out>::with_table(table.clone(), "asof_output", None)
                .await
                .expect("output dict"),
        );

        let left_state = RelationState::empty(table.clone(), "asof_left_state".to_string())
            .await
            .expect("left state");
        let right_state = RelationState::empty(table.clone(), "asof_right_state".to_string())
            .await
            .expect("right state");
        let output = VersionedZSet::new(output_dict.clone(), table.clone(), "asof_output")
            .await
            .expect("output zset");

        let left_index = LegacyIndexedBatchZSet::new(table.clone(), "asof_left_index");
        let right_index = LegacyIndexedBatchZSet::new(table.clone(), "asof_right_index");

        let left_key = Arc::new(|row: &Row| Some(row.0));
        let right_key = Arc::new(|row: &Row| Some(row.0));
        let left_ts = Arc::new(|row: &Row| row.1);
        let right_ts = Arc::new(|row: &Row| row.1);
        let projector = Arc::new(|left: &Row, right: Option<&Row>| {
            (left.0, left.1, left.2, right.map(|r| r.2))
        });

        let mut op = AsofJoinOp::new(
            left_state,
            right_state,
            left_index,
            right_index,
            left_key,
            right_key,
            left_ts,
            right_ts,
            projector,
            table.clone(),
            output,
            None,
        );

        let left_deltas: Vec<Vec<(Row, i64)>> = vec![
            vec![
                ((1, 10, 100), 1),
                ((1, 15, 101), 1),
                ((2, 5, 200), 1),
                ((3, 7, 300), 1),
            ],
            vec![((1, 12, 102), 1)],
            vec![((2, 5, 200), -1)],
            vec![],
        ];
        let right_deltas: Vec<Vec<(Row, i64)>> = vec![
            vec![
                ((1, 9, 900), 1),
                ((1, 10, 901), 1),
                ((1, 10, 905), 1),
                ((2, 4, 800), 1),
            ],
            vec![((1, 12, 910), 1)],
            vec![((1, 10, 905), -1)],
            vec![((3, 6, 700), 1)],
        ];

        let mut left_state_map: HashMap<Row, i64> = HashMap::new();
        let mut right_state_map: HashMap<Row, i64> = HashMap::new();
        let mut prev_output: HashMap<Out, i64> = HashMap::new();

        let mut cache_out = HashMap::new();
        cache_out.insert("asof_output".to_string(), output_dict.clone());

        for (step, (left_delta, right_delta)) in
            left_deltas.iter().zip(right_deltas.iter()).enumerate()
        {
            apply_deltas(&mut left_state_map, left_delta);
            apply_deltas(&mut right_state_map, right_delta);

            let output_now = recompute_asof(&left_state_map, &right_state_map);
            let expected_delta: HashMap<Out, i64> = compute_delta(&prev_output, &output_now)
                .into_iter()
                .collect();

            let left_handle = if left_delta.is_empty() {
                ZSetHandle {
                    ns: "asof_left_stream".to_string(),
                    version: 0,
                }
            } else {
                stage_version(
                    left_dict.clone(),
                    table.clone(),
                    "asof_left_stream",
                    left_delta,
                )
                .await
            };
            let right_handle = if right_delta.is_empty() {
                ZSetHandle {
                    ns: "asof_right_stream".to_string(),
                    version: 0,
                }
            } else {
                stage_version(
                    right_dict.clone(),
                    table.clone(),
                    "asof_right_stream",
                    right_delta,
                )
                .await
            };

            let out_handle = op
                .on_step(step as i64, &[left_handle, right_handle])
                .await
                .expect("asof join step");

            if expected_delta.is_empty() {
                assert!(out_handle.is_none(), "expected empty output at step {step}");
            } else {
                let out_handle = out_handle.expect("output handle");
                let materialized =
                    materialize_zset_handle::<Out>(table.clone(), &mut cache_out, &out_handle)
                        .await
                        .expect("materialize output");
                assert_eq!(materialized, expected_delta, "step {step}");
            }

            prev_output = output_now;
        }
    }
}
