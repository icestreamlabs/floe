...
use crate::operators::distinct::DistinctOp;
...
/// Distinct wrapper that drives DistinctOp over handle streams.
pub struct DbspDistinct {
        ...
        K::Archived: RkyvDeserialize<K, RkyvDeserializer> + for<'a> CheckBytes<RkyvValidator<'a>>,
    {
        ...
        let output_dict = Arc::new(
            Dictionary::<K>::with_table(table.clone(), output_ns.clone(), None)
                .await
                .context("create output dictionary for distinct")?,
        );
        ...
        let mut stream =
            build_derived_stream(table.clone(), handle_group, "distinct_output_stream/").await?;
        ...
        // Replay existing handles so the distinct output is immediately initialized.
        ...
        let mut runtime = HandleOperatorRuntime::new(vec![input.stream()], move |ts, handles| {
            let op = Arc::clone(&distinct_op);
            let writer = Arc::clone(&writer);
            let handles_vec = handles.to_vec();
            Box::pin(async move {
                if handles_vec.len() != 1 {
                    return Err(anyhow::anyhow!(
                        "distinct runtime expected 1 handle, got {}",
                        handles_vec.len()
                    ));
                }
        ...
        tokio::spawn(async move {
            loop {
                if let Err(err) = runtime.step().await {
                    report_runtime_error(&error_handler, "distinct", err);
                    break;
                }
            }
        });
...
static NEXT_DISTINCT_ID: AtomicUsize = AtomicUsize::new(0);
