...
#[tokio::test]
async fn aggregate_over_three_way_join_uses_grouped_stats_multijoin_input_semantics() {
    let people = SourceDefinition::new(
        "people",
        vec![
            SourceColumn::new_nullable("id", SourceDataType::Int64, false),
    ...
    sources.register(bids);
    let table =
        build_operator_state_table("vectorized-columnar-aggregate-over-three-way-join").await;
    let registry = Arc::new(MaterializedViewRegistry::new());
    let output_schema = Arc::new(Schema::new(vec![
        Field::new("person_id", DataType::Int64, false),
        Field::new("bid_count", DataType::Int64, false),
    ]));
    let query = "SELECT p.id AS person_id, COUNT(b.price) AS bid_count \
        FROM people p \
        JOIN auctions a ON p.id = a.seller \
        JOIN bids b ON a.id = b.auction \
        GROUP BY p.id";
    ...
    let snapshot = handle.arrow_snapshot_for(2).expect("mv snapshot");
    assert_eq!(id_count_rows(&snapshot), vec![(1, 1), (2, 1)]);
    let delta = handle.arrow_delta_for(2).expect("mv delta");
    assert_eq!(weighted_id_count_rows(&delta), vec![(2, 1, 1)]);
    ...
    let recovered_snapshot = recovered_handle
        .arrow_snapshot_for(3)
        .expect("recovered snapshot");
    assert_eq!(id_count_rows(&recovered_snapshot), vec![(1, 1), (2, 1)]);
    let recovered_delta = recovered_handle
        .arrow_delta_for(3)
        .expect("recovered empty delta");
    assert!(recovered_delta.iter().all(|batch| batch.num_rows() == 0));
    ...
    let snapshot = recovered_handle
        .arrow_snapshot_for(4)
        .expect("post-retract snapshot");
    assert_eq!(id_count_rows(&snapshot), vec![(2, 1)]);
    let delta = recovered_handle
        .arrow_delta_for(4)
        .expect("post-retract delta");
    assert_eq!(weighted_id_count_rows(&delta), vec![(1, 1, -1)]);
}
...
#[tokio::test]
async fn aggregate_over_join_topn_uses_grouped_stats_join_topn_input_semantics() {
    let auctions = SourceDefinition::new(
        "auction",
        vec![SourceColumn::new_nullable(
            "id",
    ...
    let initial_bids = RecordBatch::try_new(
        ...
        vec![
            Arc::new(Int64Array::from(vec![1, 1, 2])),
            Arc::new(Int64Array::from(vec![100, 20, 50])),
        ],
    )
    .expect("initial bid batch");
    ...
    sources.register(auctions);
    sources.register(bids);
    let table = build_operator_state_table("vectorized-columnar-aggregate-over-join-topn").await;
    let registry = Arc::new(MaterializedViewRegistry::new());
    let output_schema = Arc::new(Schema::new(vec![
        Field::new("key", DataType::Int64, false),
        Field::new("total", DataType::Int64, true),
    ]));
    let query = "SELECT key, SUM(value) AS total FROM (\
        SELECT b.auction AS key, b.price AS value \
        FROM bid b JOIN auction a ON b.auction = a.id \
        ORDER BY b.price DESC LIMIT 2) top_bids GROUP BY key";
    let mut runtime = VectorizedExecutionRuntime::new_with_options(
        &sources,
        vec![VectorizedMaterializedViewPlan::new(
            "mv_top_bid_totals",
    ...
    let handle = registry
        .get("mv_top_bid_totals")
        .expect("materialized view");
    let snapshot = handle.arrow_snapshot_for(1).expect("mv snapshot");
    assert_eq!(id_count_rows(&snapshot), vec![(1, 100), (2, 50)]);
    ...
    let snapshot = handle.arrow_snapshot_for(2).expect("mv snapshot");
    assert_eq!(id_count_rows(&snapshot), vec![(1, 100), (2, 200)]);
    let delta = handle.arrow_delta_for(2).expect("mv delta");
    assert_eq!(
        weighted_id_count_rows(&delta),
        vec![(2, 50, -1), (2, 200, 1)]
    ...
    let recovered_snapshot = recovered_handle
        .arrow_snapshot_for(3)
        .expect("recovered snapshot");
    assert_eq!(id_count_rows(&recovered_snapshot), vec![(1, 100), (2, 200)]);
    let recovered_delta = recovered_handle
        .arrow_delta_for(3)
        .expect("recovered empty delta");
    assert!(recovered_delta.iter().all(|batch| batch.num_rows() == 0));
...
#[tokio::test]
async fn grouped_aggregate_over_except_sources_uses_grouped_stats_distinct_join_input_semantics() {
    let auctions = SourceDefinition::new(
        "auction",
        vec![
            SourceColumn::new_nullable("id", SourceDataType::Int64, false),
    ...
    let initial_bids = RecordBatch::try_new(
        ...
        vec![
            Arc::new(Int64Array::from(vec![1, 1, 1, 2])),
            Arc::new(Int64Array::from(vec![100, 100, 200, 50])),
        ],
    )
    .expect("initial bid batch");
    ...
    sources.register(bids);
    let table =
        build_operator_state_table("vectorized-columnar-aggregate-over-except-sources").await;
    let registry = Arc::new(MaterializedViewRegistry::new());
    let output_schema = Arc::new(Schema::new(vec![
    ...
    let query = "SELECT key, SUM(value) AS total FROM (\
        SELECT auction AS key, price AS value FROM bid \
        EXCEPT SELECT id AS key, \"initialBid\" AS value FROM auction\
        ) s GROUP BY key";
    let mut runtime = VectorizedExecutionRuntime::new_with_options(
        &sources,
        vec![VectorizedMaterializedViewPlan::new(
    ...
    let handle = registry.get("mv_except_totals").expect("materialized view");
    let snapshot = handle.arrow_snapshot_for(1).expect("mv snapshot");
    assert_eq!(id_count_rows(&snapshot), vec![(1, 200), (2, 50)]);

    let matching_auction = RecordBatch::try_new(
        Arc::clone(&auction_schema),
        vec![
            Arc::new(Int64Array::from(vec![2])),
            Arc::new(Int64Array::from(vec![50])),
    ...
    .expect("matching auction batch");
    runtime
        .append_source_batches_for_execution_and_query(
            "auction",
            vec![matching_auction.clone()],
            vec![matching_auction],
        )
        .await
        .expect("append matching auction");
    runtime.run_tick(2).await.expect("matching auction tick");

    let snapshot = handle.arrow_snapshot_for(2).expect("mv snapshot");
    assert_eq!(id_count_rows(&snapshot), vec![(1, 200)]);
    let delta = handle.arrow_delta_for(2).expect("mv delta");
    assert_eq!(weighted_id_count_rows(&delta), vec![(2, 50, -1)]);
    ...
    let recovered_handle = recovery_registry
        .get("mv_except_totals")
        .expect("recovered materialized view");
    let recovered_snapshot = recovered_handle
        .arrow_snapshot_for(3)
        .expect("recovered snapshot");
    assert_eq!(id_count_rows(&recovered_snapshot), vec![(1, 200)]);
    let recovered_delta = recovered_handle
        .arrow_delta_for(3)
        .expect("recovered empty delta");
    assert!(recovered_delta.iter().all(|batch| batch.num_rows() == 0));

    let weighted_schema = crate::delta_consolidation::weighted_snapshot_schema(&auction_schema)
        .expect("weighted schema");
    let retract = RecordBatch::try_new(
        Arc::clone(&auction_schema),
        vec![
    ...
    .expect("retract auction batch");
    let weighted = weighted_batch_from_diffs(&retract, &weighted_schema, &[-1])
        .expect("weighted retract auction");
    recovered
        .apply_weighted_source_delta("auction", weighted)
        .await
...
#[tokio::test]
async fn global_aggregate_over_intersect_sources_uses_grouped_stats_distinct_join_input_semantics()
{
    let auctions = SourceDefinition::new(
        "auction",
        vec![
    ...
    sources.register(bids);
    let table =
        build_operator_state_table("vectorized-columnar-aggregate-over-intersect-sources").await;
    let registry = Arc::new(MaterializedViewRegistry::new());
    let output_schema = Arc::new(Schema::new(vec![Field::new(
        "total",
        DataType::Int64,
    ...
    let query = "SELECT SUM(value) AS total FROM (\
        SELECT auction AS key, price AS value FROM bid \
        INTERSECT SELECT id AS key, \"initialBid\" AS value FROM auction\
        ) s";
    let mut runtime = VectorizedExecutionRuntime::new_with_options(
        &sources,
        vec![VectorizedMaterializedViewPlan::new(
    ...
    let handle = registry
        .get("mv_intersect_total")
        .expect("materialized view");
    let snapshot = handle.arrow_snapshot_for(1).expect("mv snapshot");
    assert_eq!(single_int_rows(&snapshot), vec![100]);
    ...
    let snapshot = handle.arrow_snapshot_for(2).expect("mv snapshot");
    assert_eq!(single_int_rows(&snapshot), vec![150]);
    let delta = handle.arrow_delta_for(2).expect("mv delta");
    assert_eq!(weighted_single_int_rows(&delta), vec![(100, -1), (150, 1)]);
    ...
    let recovered_snapshot = recovered_handle
        .arrow_snapshot_for(3)
        .expect("recovered snapshot");
    assert_eq!(single_int_rows(&recovered_snapshot), vec![150]);
    let recovered_delta = recovered_handle
        .arrow_delta_for(3)
        .expect("recovered empty delta");
    assert!(recovered_delta.iter().all(|batch| batch.num_rows() == 0));
...
#[tokio::test]
async fn grouped_count_join_uses_slate_backed_columnar_join_operator_semantics() {
    let bids = SourceDefinition::new(
        "bids",
        vec![
            SourceColumn::new_nullable("auction", SourceDataType::Int64, false),
    ...
    sources.register(bids);
    sources.register(auctions);
    let table = build_operator_state_table("vectorized-columnar-grouped-count-join").await;
    let registry = Arc::new(MaterializedViewRegistry::new());
    let output_schema = Arc::new(Schema::new(vec![
        Field::new("auction", DataType::Int64, false),
        Field::new("bid_count", DataType::Int64, false),
    ...
    let query = "SELECT c.auction, c.bid_count, a.seller \
        FROM (SELECT auction, COUNT(*) AS bid_count FROM bids GROUP BY auction) c \
        JOIN auctions a ON c.auction = a.id";
    let mut runtime = VectorizedExecutionRuntime::new_with_options(
        &sources,
        vec![VectorizedMaterializedViewPlan::new(
            "mv_auction_bid_counts",
            query,
            Arc::clone(&output_schema),
        )],
        Arc::clone(&registry),
    ...
    let handle = registry
        .get("mv_auction_bid_counts")
        .expect("materialized view");
    let snapshot = handle.arrow_snapshot_for(1).expect("mv snapshot");
    assert_eq!(self_join_rows(&snapshot), vec![(1, 2, 10), (2, 1, 20)]);
    ...
    let snapshot = handle.arrow_snapshot_for(2).expect("mv snapshot");
    assert_eq!(self_join_rows(&snapshot), vec![(1, 2, 10), (2, 2, 20)]);
    let delta = handle.arrow_delta_for(2).expect("mv delta");
    assert_eq!(
        weighted_self_join_rows(&delta),
        vec![(2, 1, 20, -1), (2, 2, 20, 1)]
    );
    ...
    let snapshot = recovered_handle
        .arrow_snapshot_for(4)
        .expect("post-retract snapshot");
    assert_eq!(self_join_rows(&snapshot), vec![(1, 1, 10), (2, 2, 20)]);
    let delta = recovered_handle
        .arrow_delta_for(4)
        .expect("post-retract delta");
    assert_eq!(
        weighted_self_join_rows(&delta),
        vec![(1, 1, 10, 1), (1, 2, 10, -1)]
    );
}

#[tokio::test]
async fn topn_over_aggregate_join_uses_slate_backed_columnar_topn_operator_semantics() {
    let bids = SourceDefinition::new(
        "bids",
        vec![
            SourceColumn::new_nullable("auction", SourceDataType::Int64, false),
    ...
    sources.register(bids);
    sources.register(auctions);
    let table = build_operator_state_table("vectorized-columnar-topn-over-aggregate-join").await;
    let registry = Arc::new(MaterializedViewRegistry::new());
    let output_schema = Arc::new(Schema::new(vec![
        Field::new("auction", DataType::Int64, false),
        Field::new("max_price", DataType::Int64, false),
    ]));
    let query = "SELECT j.auction, j.max_price \
        FROM ( \
            SELECT m.auction, m.max_price, a.seller \
            FROM (SELECT auction, MAX(price) AS max_price FROM bids GROUP BY auction) m \
            JOIN auctions a ON m.auction = a.id \
        ) j \
        ORDER BY j.max_price DESC LIMIT 2";
    let mut runtime = VectorizedExecutionRuntime::new_with_options(
        &sources,
        vec![VectorizedMaterializedViewPlan::new(
            "mv_top_aggregate_join",
            query,
            Arc::clone(&output_schema),
        )],
        Arc::clone(&registry),
    ...
    let snapshot = handle.arrow_snapshot_for(2).expect("mv snapshot");
    assert_eq!(id_count_rows(&snapshot), vec![(2, 200), (3, 250)]);
    let delta = handle.arrow_delta_for(2).expect("mv delta");
    assert_eq!(
        weighted_id_count_rows(&delta),
        vec![(1, 100, -1), (3, 250, 1)]
    ...
    let recovered_snapshot = recovered_handle
        .arrow_snapshot_for(3)
        .expect("recovered snapshot");
    assert_eq!(id_count_rows(&recovered_snapshot), vec![(2, 200), (3, 250)]);
    let recovered_delta = recovered_handle
        .arrow_delta_for(3)
        .expect("recovered empty delta");
    assert!(recovered_delta.iter().all(|batch| batch.num_rows() == 0));
...
#[tokio::test]
async fn join_over_join_aggregate_uses_slate_backed_columnar_join_operator_semantics() {
    let bids = SourceDefinition::new(
        "bids",
        vec![
            SourceColumn::new_nullable("auction", SourceDataType::Int64, false),
    ...
    let initial_people = RecordBatch::try_new(
        Arc::clone(&people_schema),
        vec![
            Arc::new(Int64Array::from(vec![300, 350])),
            Arc::new(StringArray::from(vec!["total-300", "total-350"])),
        ],
    )
    .expect("initial people batch");
    ...
    sources.register(auctions);
    sources.register(people);
    let table = build_operator_state_table("vectorized-columnar-join-aggregate-join").await;
    let registry = Arc::new(MaterializedViewRegistry::new());
    let output_schema = Arc::new(Schema::new(vec![
        Field::new("total_price", DataType::Int64, true),
        Field::new("name", DataType::Utf8, false),
    ]));
    let query = "SELECT j.total_price, p.name \
        FROM ( \
            SELECT b.auction, SUM(b.price) AS total_price \
            FROM bids b JOIN auctions a ON b.auction = a.id \
            GROUP BY b.auction \
        ) j \
        JOIN people p ON j.total_price = p.id";
    let mut runtime = VectorizedExecutionRuntime::new_with_options(
        &sources,
        vec![VectorizedMaterializedViewPlan::new(
            "mv_join_aggregate_people",
            query,
            Arc::clone(&output_schema),
        )],
        Arc::clone(&registry),
    ...
    let handle = registry
        .get("mv_join_aggregate_people")
        .expect("materialized view");
    let snapshot = handle.arrow_snapshot_for(1).expect("mv snapshot");
    assert_eq!(
        id_note_rows(&snapshot),
        vec![(300, "total-300".to_string())]
    );
    ...
    assert_eq!(
        id_note_rows(&snapshot),
        vec![(350, "total-350".to_string())]
    );
    let delta = handle.arrow_delta_for(2).expect("mv delta");
    assert_eq!(
        weighted_id_note_rows(&delta),
        vec![
            (300, "total-300".to_string(), -1),
            (350, "total-350".to_string(), 1)
        ]
    );
...
#[tokio::test]
async fn global_stats_join_uses_slate_backed_columnar_join_operator_semantics() {
    let bids = SourceDefinition::new(
        "bids",
        vec![
            SourceColumn::new_nullable("auction", SourceDataType::Int64, false),
    ...
    sources.register(bids);
    sources.register(people);
    let table = build_operator_state_table("vectorized-columnar-global-stats-join").await;
    let registry = Arc::new(MaterializedViewRegistry::new());
    let output_schema = Arc::new(Schema::new(vec![
        Field::new("total_price", DataType::Int64, true),
        Field::new("name", DataType::Utf8, false),
    ]));
    let query = "SELECT g.total_price, p.name \
        FROM (SELECT SUM(price) AS total_price FROM bids) g \
        JOIN people p ON g.total_price = p.id";
    let mut runtime = VectorizedExecutionRuntime::new_with_options(
        &sources,
        vec![VectorizedMaterializedViewPlan::new(
            "mv_total_price_people",
            query,
            Arc::clone(&output_schema),
        )],
        Arc::clone(&registry),
