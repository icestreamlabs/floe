...
pub(super) fn record_since(label: &'static str, start: Option<Instant>) {
...
pub(super) fn record(label: &'static str, elapsed: Duration) {
    if !enabled() {
        return;
    }
...
pub fn reset_columnar_phase_profile() {
    if !enabled() {
        return;
    }
    PHASE_PROFILE
...
pub fn print_columnar_phase_profile(name: &str) {
    ...
    for (label, metric) in metrics {
        eprintln!(
            "[columnar-phase-profile] name={} phase={} calls={} total_ms={:.3} mean_ms={:.3}",
            name,
            label,
            metric.calls,
