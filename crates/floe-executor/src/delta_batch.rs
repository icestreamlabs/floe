...
impl DeltaBatchBuffer {
    pub fn new(base_schema: SchemaRef, config: DeltaBatchConfig) -> Result<Self> {
        if base_schema.index_of(WEIGHT_COLUMN_NAME).is_ok() {
            bail!("base schema must not contain reserved delta column {WEIGHT_COLUMN_NAME}");
        }
    ...
    pub fn push_ref(&mut self, row: &[u8], weight: Diff) -> Result<Option<RecordBatch>> {
        if weight == 0 {
            return Ok(None);
        }
...
mod tests {
    ...
    fn flushes_on_row_threshold() {
        ...
        let mut buffer = DeltaBatchBuffer::new(schema(), config).expect("buffer");
        assert!(buffer.push_ref(&row(1, "a"), 1).unwrap().is_none());
        let batch = buffer.push_ref(&row(2, "b"), 1).unwrap().unwrap();
    ...
    fn flushes_on_byte_threshold() {
        ...
        let mut buffer = DeltaBatchBuffer::new(schema(), config).expect("buffer");
        let batch = buffer.push_ref(&row(1, "a"), 1).unwrap().unwrap();
