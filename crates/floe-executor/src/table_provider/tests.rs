...
async fn source_provider_emits_rows() {
    let store: Arc<dyn ObjectStore> = Arc::new(InMemory::new());
    let db = Arc::new(Db::open("source-provider", store).await.expect("open SlateDB"));
    ...
    let mut stream = bridge
        .new_stream(namespace.clone(), dbsp::StreamRetention::KeepLast { keep_last: 1 })
        .await
        .expect("stream");
    let row = vec![
    ...
    let provider =
        SourceTableProvider::new(bridge, "nexmark_bid", "nexmark_bid", schema).unwrap();
    let batches = provider
        .build_batches_for_test()
        .await
...
fn mv_version_filter_is_extracted() {
    ...
    let (none_version, unchanged) =
        extract_mv_version_filter(std::slice::from_ref(&other_filter));
