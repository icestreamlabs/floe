...
fn hopping_window_count_plan() -> dbsp::CircuitPlan {
...
fn tumbling_window_max_plan() -> dbsp::CircuitPlan {
    let bid = nexmark_bid_table();
    let input_schema = bid.schema().clone();
    let aggregate = dbsp::DbspAggregateNode::try_new(
        input_schema.clone(),
...
fn tumbling_window_count_by_bidder_plan() -> dbsp::CircuitPlan {
    let bid = nexmark_bid_table();
    let input_schema = bid.schema().clone();
    let aggregate = dbsp::DbspAggregateNode::try_new(
        input_schema.clone(),
...
async fn build_window_plan_rows(
    db_name: &str,
    view_name: &str,
...
fn root_arrow_schema(plan: &dbsp::CircuitPlan) -> Arc<Schema> {
    plan.node(plan.root)
        .map(|node| node.output_schema.to_arrow_schema())
        .expect("root node")
}

fn row_contains_i64(row: &TestRow, expected: i64) -> bool {
    row.iter().any(|value| {
        matches!(
            value,
